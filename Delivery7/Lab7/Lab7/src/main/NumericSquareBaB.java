...
public class NumericSquareBaB {
	...
	public NumericSquareBaB(String[][] square) {
		this.square = square;
	...
	private boolean computeRec(List<Point> editablePoints, int step) {
		...
		for (int i = 0; i <= 9; i++) {
			...
			if (prun(stateSquare[x], y) || prun(getColumn(stateSquare, y), x)) 
				continue;
	...
	private boolean prun(String[] operation, int i) {
		i++;

		if (!(operation[i + 1].equals("?")) && i < operation.length - 2) 
			switch (operation[i]) {
//			case "+":
//				if (checkIncrementalOperation(Integer.parseInt(operation[i - 1]), Integer.parseInt(operation[i + 1]),
//						Integer.parseInt(operation[operation.length - 1])))
//					return true;
//				break;
//			case "-":
//				if (checkDecrementalOperation(Integer.parseInt(operation[i - 1]), Integer.parseInt(operation[i + 1]),
//						Integer.parseInt(operation[operation.length - 1])))
//					return true;
//				break;
			case "/":
				if (checkDivisionOperation(Integer.parseInt(operation[i - 1]), Integer.parseInt(operation[i + 1]),
						Integer.parseInt(operation[operation.length - 1])))
					return true;
				break;
//			case "*":
...
//				break;
	...
	private boolean checkIncrementalOperation(int operand1, int operand2, int sol) {

		// * or +: if operand1 or operand2 are greater than sol, probably it wont be a
		// solution

		return (operand1 > sol || operand2 > sol);
	}

	private boolean checkDecrementalOperation(int operand1, int operand2, int sol) {

		// - : if operand1 is greater than sol, probably it wont be a
		// solution

		return (operand1 < sol);
	}

	private boolean checkDivisionOperation(int operand1, int operand2, int sol) {

		// /: if operand1 is greater than sol, probably it wont be a
		// solution and also the usual conditions for division

		return operand1 < operand2 || operand2 == 0 || operand1 % operand2 != 0;
	}
